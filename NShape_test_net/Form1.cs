...
namespace StatesDiagram2
{
    public partial class Form1 : Form
    {
        ...
        private Microsoft.Msagl.GraphViewerGdi.LayoutMethod _currentLayoutMode = Microsoft.Msagl.GraphViewerGdi.LayoutMethod.SugiyamaScheme;
        private volatile bool _isPanning = false;
        ...
        {
            checkBox_useShelf.Checked = Properties.Settings.Default.useShelf;
            ...
            checkBox_showStateElements.Checked = _showStateElements = Properties.Settings.Default.ShowStateElements;
        ...
        private void LoadStates(IEnumerable<string> filesList)
        {
            ...
            foreach (var file in filesList)
            {
                ...
                if (Properties.Settings.Default.useShelf)
            ...
            CreateLinks(graph, states, links);
        ...
        {
            Properties.Settings.Default.useShelf = checkBox_useShelf.Checked;
        ...
        private void GViewer1_ObjectUnderMouseCursorChanged(object sender, ObjectUnderMouseCursorChangedEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    if (node.UserData is ShapeTag tag)
                    {
                        textBox_tag.Text = tag?.ToString();
                    }
                }
        ...
        private void GViewer1_DoubleClick(object sender, EventArgs e)
        {
            if (sender is Microsoft.Msagl.GraphViewerGdi.GViewer obj && obj.SelectedObject != null)
            {
                ...
                else if (obj.SelectedObject is Node node)
                {
                    if (node.UserData is ShapeTag t)
                        tag = t;
                }
        ...
        private void ComboBox_layoutMode_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (Enum.TryParse<Microsoft.Msagl.GraphViewerGdi.LayoutMethod>(newValue, out var layoutMode))
            {
        ...
        {
            Properties.Settings.Default.ShowStateElements = _showStateElements = checkBox_showStateElements.Checked;
        }
        ...
        private void GetStateShelf(IEnumerable<ParsedProperty> pathList, string filePath, out State state, out List<StateLink> stateLinks)
        {
            ...
            foreach (var button in buttons)
            {
                ...
                var methodsNames = pathList.Where(n => (n.Name.Equals("proceed", StringComparison.OrdinalIgnoreCase) || n.Name.Equals("complete", StringComparison.OrdinalIgnoreCase) || n.Name.Equals("actionToPerform", StringComparison.OrdinalIgnoreCase)) && n.ParentPath.Equals(button.Path, StringComparison.OrdinalIgnoreCase)).ToList();
            ...
            if (nextStateTransitions != null && nextStateTransitions.Any())
            {
                foreach (var cState in nextStateTransitions)
                {
                    ...
                    {
                        // get the name of the method which is running the transition for description
                        var actionName = pathList.FirstOrDefault(n => n.Name.Equals("name", StringComparison.OrdinalIgnoreCase) && n.ParentPath.Equals(TrimPathEnd(transition.ParentPath, 2, _pathDivider), StringComparison.OrdinalIgnoreCase));

                        var newLink = new StateLink()
                        {
                            FromState = state.Id,
                            FromAction = actionName.Value,
            ...
            if (returnLinks != null && returnLinks.Any())
            {
        ...
        List<StateButton> FindInternalButtons(IEnumerable<ParsedProperty> pathList, string filePath, ParsedProperty startProperty)
        {
        ...
        private void GetStateNlmk(IEnumerable<ParsedProperty> pathList, string filePath, out State state, out List<StateLink> stateLinks)
        {
            ...
            foreach (var transition in transitions)
            {
                ...
                if (string.IsNullOrEmpty(actionNameStr))
                {
                    var i = actionName.Path.LastIndexOf(_pathDivider);
                    if (i > 1)
                    {
        ...
        private void CreateLinks(Graph graph, List<State> states, List<StateLink> links)
        {
        ...
        private void FixStateLinks(ref List<StateLink> links, List<State> states)
        {
            foreach (var link in links)
            {
        ...
        List<State> FindOrphanStates(List<State> states, List<StateLink> links)
        {
        ...
        private void FindBiDirectionalLinks(List<State> states, ref List<StateLink> links)
        {
            foreach (var link in links)
            {
